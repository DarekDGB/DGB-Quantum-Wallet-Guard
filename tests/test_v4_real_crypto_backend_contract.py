...
def real_key(*, algorithm: str = "ml-dsa", public_key: str | None = None) -> dict[str, Any]:
    return {
        "role": COMPONENT_ROLE,
        "key_id": f"shield_component_qwg-{algorithm}-v1",
        "key_version": 1,
        "algorithm": algorithm,
...
def test_v48g_r4_qwg_real_crypto_signature_input_rejects_ambiguous_values(
    ...
    base.update(kwargs)
    with pytest.raises((QwgV4RealCryptoBackendError, ValueError), match=match):
...
def test_v48g_r4_qwg_real_crypto_verifier_fails_closed_on_test_key_material_and_key_mismatch() -> None:
    ...
    test_key_id["key_id"] = "test-shield_component_qwg-ml-dsa-v1"
    test_entry = signature_for_key(real_key())
    test_entry["key_id"] = "test-shield_component_qwg-ml-dsa-v1"
    with pytest.raises(QwgV4RealCryptoMaterialError, match="test-only"):
    ...
    with pytest.raises(QwgV4RealCryptoBackendError, match="role"):
    ...
    with pytest.raises(QwgV4RealCryptoBackendError, match="registry key"):
...
def test_v48g_r4_qwg_real_crypto_verifier_rejects_bad_entry_and_backend_gap_before_verify() -> None:
    ...
    with pytest.raises(QwgV4RealCryptoBackendError, match="domain_tag"):
    ...
    with pytest.raises(QwgV4RealCryptoBackendError, match="lowercase"):
...
def test_v48g_r4_qwg_real_binary_encoding_helpers_are_strict() -> None:
    ...
    with pytest.raises(QwgV4RealCryptoBackendError, match="bytes"):
        encode_binary_signature_material(b"", field="signature")
    with pytest.raises(QwgV4RealCryptoBackendError, match="b64u"):
        decode_binary_signature_material("abc", field="signature")
    with pytest.raises(QwgV4RealCryptoBackendError, match="non-empty"):
        decode_binary_signature_material("b64u:", field="signature")
    with pytest.raises(QwgV4RealCryptoBackendError, match="unpadded"):
