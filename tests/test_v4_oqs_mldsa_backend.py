...
import qwg.v4.oqs_mldsa_backend as oqs_backend_module
from qwg.v4.oqs_mldsa_backend import OQS_ML_DSA_MECHANISM, OqsMlDsaBackend
...
class NonBoolVerify(FakeOqsSignature):
    def verify(self, message: bytes, signature: bytes, public_key: bytes) -> bool:  # type: ignore[override]
...
class SignatureConstructorFailureModule(FakeOqsModule):
    @property
    def Signature(self) -> type[FakeOqsSignature]:
        raise NativeOqsError("native signature constructor failure")


def failing_resolver(reference: str) -> bytes:
    raise NativeOqsError("native resolver failure")


def resolver(reference: str) -> bytes:
    if reference == "hsm://qwg/ml-dsa/v1":
...
def test_v48e_oqs_mldsa_backend_builds_real_b64u_signature_entry_and_verifies() -> None:
...
def test_v48e_oqs_mldsa_backend_rejects_wrong_algorithm_and_mechanism() -> None:
    with pytest.raises(QwgV4RealCryptoBackendError, match="private_key_resolver"):
        OqsMlDsaBackend(private_key_resolver="not-callable")  # type: ignore[arg-type]
    with pytest.raises(QwgV4RealCryptoBackendError, match="ML-DSA-65"):
    ...
    with pytest.raises(QwgV4RealCryptoBackendUnavailable, match="ml-dsa"):
...
def test_v48e_oqs_mldsa_backend_fails_closed_when_oqs_missing_or_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(QwgV4RealCryptoBackendUnavailable, match="import oqs"):
    ...
    with pytest.raises(QwgV4RealCryptoBackendUnavailable, match="not enabled"):
...
def test_v48e_oqs_mldsa_backend_rejects_bad_binary_material() -> None:
    backend = OqsMlDsaBackend(private_key_resolver=resolver, oqs_module=FakeOqsModule())
    with pytest.raises(QwgV4RealCryptoBackendError, match="message"):
        backend.sign_message(
            algorithm="ml-dsa",
            private_key_reference="hsm://qwg/ml-dsa/v1",
            message=b"",
        )
    with pytest.raises(QwgV4RealCryptoBackendError, match="secret_key"):
        backend.sign_message(algorithm="ml-dsa", private_key_reference="unknown", message=b"message")
    with pytest.raises(QwgV4RealCryptoBackendError, match="public_key"):
        backend.verify_signature(
            algorithm="ml-dsa",
            public_key="not-b64u",
            message=b"message",
    ...
    with pytest.raises(QwgV4RealCryptoBackendError, match="signature"):
        backend.verify_signature(
            algorithm="ml-dsa",
            public_key=real_key()["public_key"],  # type: ignore[arg-type]
            message=b"message",
...
def test_v48e_oqs_mldsa_backend_rejects_empty_signature_bytes() -> None:
    backend = OqsMlDsaBackend(private_key_resolver=resolver, oqs_module=FakeOqsModule(signature_cls=EmptySignature))
    with pytest.raises(QwgV4RealCryptoBackendError, match="signature"):
        backend.sign_message(algorithm="ml-dsa", private_key_reference="hsm://qwg/ml-dsa/v1", message=b"message")


def test_v48e_oqs_mldsa_backend_wraps_native_oqs_exceptions_fail_closed() -> None:
    backend = OqsMlDsaBackend(private_key_resolver=resolver, oqs_module=VersionDiscoveryFailureModule())
    with pytest.raises(QwgV4RealCryptoBackendError, match="version discovery failed closed") as version_error:
        _ = backend.backend_version
    ...
    with pytest.raises(QwgV4RealCryptoBackendError, match="mechanism discovery failed closed") as mechanism_error:
        backend.sign_message(algorithm="ml-dsa", private_key_reference="hsm://qwg/ml-dsa/v1", message=b"message")
...
def test_v48e_oqs_binary_encoding_helpers_match_real_backend_contract() -> None:
...
def test_v48g_qwg_oqs_mldsa_backend_wraps_import_resolver_constructor_and_rejects_non_bool_verify(
    ...
    with pytest.raises(QwgV4RealCryptoBackendError, match="import failed closed") as import_error:
        _ = backend.backend_version
    ...
    with pytest.raises(QwgV4RealCryptoBackendError, match="private key resolution failed closed") as resolver_error:
        backend.sign_message(algorithm="ml-dsa", private_key_reference="hsm://qwg/ml-dsa/v1", message=b"message")
    ...
    with pytest.raises(QwgV4RealCryptoBackendError, match="sign failed closed") as constructor_error:
        backend.sign_message(algorithm="ml-dsa", private_key_reference="hsm://qwg/ml-dsa/v1", message=b"message")
...
def test_v48g_qwg_oqs_mldsa_backend_rejects_wrong_binary_lengths_before_native_verify() -> None:
    backend = OqsMlDsaBackend(
        private_key_resolver=resolver,
        oqs_module=FakeOqsModule(signature_cls=LengthCheckedOqsSignature),
    )

    with pytest.raises(QwgV4RealCryptoBackendError, match="public_key byte length"):
        backend.verify_signature(
            algorithm="ml-dsa",
    ...
    with pytest.raises(QwgV4RealCryptoBackendError, match="signature byte length"):
        backend.verify_signature(
            algorithm="ml-dsa",
            public_key=real_key()["public_key"],  # type: ignore[arg-type]
            message=b"message",
...
def test_v48g_qwg_oqs_mldsa_backend_rejects_direct_test_only_private_reference() -> None:
    backend = OqsMlDsaBackend(private_key_resolver=resolver, oqs_module=FakeOqsModule())
    with pytest.raises(QwgV4RealCryptoBackendError, match="private_key_reference"):
        backend.sign_message(algorithm="ml-dsa", private_key_reference="test-only-private", message=b"message")
...
def test_v48g_qwg_oqs_mldsa_backend_validates_optional_backend_length_metadata() -> None:
    backend = OqsMlDsaBackend(
        private_key_resolver=resolver,
        oqs_module=FakeOqsModule(signature_cls=NonMappingDetailsOqsSignature),
    )
    with pytest.raises(QwgV4RealCryptoBackendError, match="details must be a mapping"):
        backend.verify_signature(
            algorithm="ml-dsa",
            public_key=real_key()["public_key"],  # type: ignore[arg-type]
            message=b"message",
    ...
    with pytest.raises(QwgV4RealCryptoBackendError, match="length_public_key"):
        backend.verify_signature(
            algorithm="ml-dsa",
            public_key=real_key()["public_key"],  # type: ignore[arg-type]
            message=b"message",
