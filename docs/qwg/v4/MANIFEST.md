# QWG Shield v4 Manifest and Trust Profile
...
```text
project: dgb-quantum-wallet-guard
...
Canonical bundle order is exactly classical, ML-DSA, then optional FN-DSA.
Received bundles are not repaired or reordered.

## Trust entry schema

Each QWG trust entry requires:
...
Only `shield_component_qwg` is valid. The algorithm remains separate from the
role. Wrong-role, unknown, revoked, expired, not-yet-valid, wrong-algorithm, or
...
QWG does not sign or broadcast transactions, hold wallet keys, modify
consensus, approve execution, produce the final Shield receipt, bypass the
Orchestrator, or override AdamantineOS. AdamantineOS remains the final
